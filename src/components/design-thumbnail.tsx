...
const designs: Record<number, React.ComponentType> = {
  1: Design01,
  2: Design02,
  3: Design03,
  4: Design04,
...
const REF_WIDTH = 1920 / 1.3;
const REF_HEIGHT = 1080 / 1.3;

export default function DesignThumbnail({
  designId,
  className = "",
}: {
  designId: number | "default";
  className?: string;
}) {
  const Component = designId === "default" ? DefaultHome : designs[designId];
  ...
  return (
    <div ref={containerRef} className={`relative overflow-hidden ${className}`}>
      <div
        className="no-animations pointer-events-none absolute inset-0 origin-top-left *:h-full! *:min-h-full! *:w-full!"
        style={{
          width: REF_WIDTH,
          height: REF_HEIGHT,
